################################################################################################################
#Overview of the API Gateway-Fusion Auth Deployment of Microservices With Diff Architecture(Services-Integrated)
################################################################################################################
#Directory Structure:
#########################
...
If you plan to run MSSQL directly on EC2, you'll need a Windows Server AMI with SQL Server pre-installed, 
or install it yourself via user_data. For a simple test, a basic Linux AMI with Nginx is used as a placeholder.

You can find AMIs in the AWS EC2 console under "Launch Instances" -> "AMIs", or using the AWS CLI: 
aws ec2 describe-images --owners amazon --filters "Name=name,Values=amzn2-ami-hvm-*-x86_64-gp2" --query 'sort_by(Images, &CreationDate)[-1].ImageId' for Amazon Linux 2.
########################################################################################################################################
...
Replace placeholders with your actual values. For sensitive variables like passwords and API keys, consider using a terraform.tfvars
...
Test the API:
#######################
After terraform apply completes, you will get outputs for api_gateway_base_url, api_key_value, and the DNS names/endpoints of your 
backend services.
...
rds_postgres_sg / rds_mssql_sg: Restricts inbound database traffic only to the respective application security groups
(ECS for Postgres, EC2 for MSSQL).
###########################################################################
Payment & Telemedicine Services (API Gateway -> SQS -> Lambda -> DynamoDB):
############################################################################
SQS Queues (aws_sqs_queue): Act as asynchronous buffers. API Gateway sends messages to SQS, decoupling the API call from 
the backend processing. This improves responsiveness and resilience.
########################################################################################################################
...
--------------------------------------------------------------------------------------
request_templates: This is vital. API Gateway receives a JSON request, but 
SQS's SendMessage action expects a specific x-www-form-urlencoded format. 
The template transforms the incoming JSON body into the MessageBody parameter for SQS. 
MessageDeduplicationId and MessageGroupId are included for FIFO queues
(though these are standard SQS queues, these parameters are still valid).
----------------------------------------------------------------------------------------
################################################################################################################################
Lambda SQS Event Source Mapping (aws_lambda_event_source_mapping): Configures the Lambda function to automatically poll 
the SQS queue and invoke the Lambda with new messages.
###############################################################################################################################
DynamoDB Tables (aws_dynamodb_table): NoSQL databases for high-performance, scalable data storage.
###################################################################################################
Lambda Code (main.py for SQS Lambdas): These Lambdas are triggered by SQS messages.
They parse the message body (which is URL-encoded JSON from API Gateway), and then interact with DynamoDB using boto3.
########################################################################################################################
Pharmacy Service (API Gateway -> ALB -> ECS -> PostgreSQL):
##############################################################
ECS Cluster (aws_ecs_cluster): A logical grouping for your ECS services.

ECS Task Definition (aws_ecs_task_definition): Defines your application container, CPU/memory, network mode (awsvpc for Fargate), 
and environment variables (e.g., for database connection details). You'll need to replace nginx:latest with your actual microservice
Docker image.
...
ALB Listener (aws_lb_listener): Checks for connection requests from clients, using the protocol and port that you configure, and 
forwards requests to a target group.

RDS PostgreSQL (aws_db_instance): A managed relational database service. 
It's placed in private subnets and secured by rds_postgres_sg to only allow connections from ECS tasks.

API Gateway ALB Integration (type = "HTTP_PROXY", uri to ALB DNS): API Gateway acts as a direct proxy to the ALB. 
The {proxy} variable in the URI ensures that the full path from the API Gateway request is passed through to the ALB and
then to the ECS service.
####################################################################################################################################
Personal Health Management Service (API Gateway -> ALB -> EC2 -> MSSQL):
###########################################################################

EC2 Instance (aws_instance): A virtual server where your PFM microservice would run. It's placed in a private subnet and attached to
the ec2_instances_sg.

RDS MSSQL (aws_db_instance): Managed SQL Server database. Similar to PostgreSQL, it's in private subnets and secured by rds_mssql_sg 
to only allow connections from the EC2 instance.

ALB Integration: Works identically to the Pharmacy service, routing traffic from API Gateway to the ALB, which then forwards to 
the EC2 instance.

user_data for EC2: This script runs when the EC2 instance first launches. You would use it to install your application, web server
(like IIS for Windows or Nginx/Apache for Linux), and configure it to connect to the MSSQL database.
######################################################################################################################################
Response Processing:
################################
For SQS integrations, the API Gateway immediately returns a 200 OK if the message is successfully sent to SQS. 
The actual processing and any eventual response to the client (if needed for asynchronous operations) would be handled by a separate 
mechanism (e.g., WebSockets, polling, or callbacks).
...
Scalability:
#####################
All AWS services used (API Gateway, SQS, Lambda, DynamoDB, ALB, ECS) are inherently scalable and managed by AWS, 
...
This comprehensive Terraform setup provides a robust and scalable architecture for your microservices, adhering to all your specified 
requirements. Remember to replace placeholder values and images with your actual application details.