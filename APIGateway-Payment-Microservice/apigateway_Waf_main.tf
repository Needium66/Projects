...
# -----------------------------------------------------------------------------
# 6. API Gateway Models and Validators (Input Validation)
# -----------------------------------------------------------------------------

# Model for the POST /payments request body
resource "aws_api_gateway_model" "payment_request_model" {
  rest_api_id  = aws_api_gateway_rest_api.payment_api_gateway.id
  name         = "PaymentRequest"
  description  = "Model for creating a new payment"
...
# Request validator for POST /payments
resource "aws_api_gateway_request_validator" "payment_request_validator" {
  rest_api_id                = aws_api_gateway_rest_api.payment_api_gateway.id
  name                       = "PaymentRequestValidator"
  validate_request_body      = true
  validate_request_parameters = false # Only validating body for now
}
...
resource "aws_api_gateway_method" "post_payments_method" {
  ...
  request_validator_id = aws_api_gateway_request_validator.payment_request_validator.id

  request_models = {
    "application/json" = aws_api_gateway_model.payment_request_model.name
  }
}
...
resource "aws_api_gateway_deployment" "payment_api_deployment" {
  ...
  triggers = {
    redeployment = sha1(jsonencode([
      ...
      aws_api_gateway_model.payment_request_model.id,       # Include new model
      aws_api_gateway_request_validator.payment_request_validator.id # Include new validator
    ]))
  }
...
resource "aws_wafv2_web_acl" "payment_api_waf" {
  name        = "PaymentServiceAPI-WebACL"
  scope       = "REGIONAL" # Or "CLOUDFRONT" if using Edge optimized API Gateway
  description = "WAF for Payment Service API Gateway"
  default_action {
    allow {} # Default to allow requests not explicitly blocked by rules
  }

  # Rule: Rate Limiting (example: 1000 requests per 5 minutes from a single IP)
  rule {
    name     = "RateLimitRule"
    priority = 10
    action {
  ...
  # Rule: AWS Managed Rule Group for Common Vulnerabilities
  rule {
    name     = "AWSManagedRulesCommon"
    priority = 20
    override_action {
      count {} # Count requests that match, but don't block by default
    }
    statement {
      managed_rule_group_statement {
        vendor_name = "AWS"
        name        = "AWSManagedRulesCommonRuleSet"
        # Optional: excluded_rules if you want to exclude specific rules from this managed set
      }
    }
    visibility_config {
      cloudwatch_metrics_enabled = true
  ...
  # Rule: AWS Managed Rule Group for SQL Injection
  rule {
  ...
  # Rule: AWS Managed Rule Group for XSS
  rule {
    ...
    statement {
      managed_rule_group_statement {
        vendor_name = "AWS"
        name        = "AWSManagedRulesKnownBadInputsRuleSet" # This set includes XSS
      }
    }
    visibility_config {
      cloudwatch_metrics_enabled = true
      metric_name                = "AWSManagedRulesXSSMetric"
      sampled_requests_enabled   = true
    }
  }

  tags = {
    Name        = "PaymentServiceAPI-WAF"
    Environment = "dev"
    Service     = "PaymentService"
  }
}
...
resource "aws_wafv2_web_acl_association" "payment_api_waf_association" {
  resource_arn = aws_api_gateway_stage.dev_stage.arn
  web_acl_arn  = aws_wafv2_web_acl.payment_api_waf.arn
}

# -----------------------------------------------------------------------------
# 8. API Gateway Usage Plan and API Key (Rate Limiting)
# -----------------------------------------------------------------------------

# API Key
resource "aws_api_gateway_api_key" "payment_api_key" {
  name        = "PaymentServiceApiKey"
  description = "API Key for Payment Service access"
  enabled     = true
...
resource "aws_api_gateway_usage_plan" "payment_usage_plan" {
  name        = "PaymentServiceUsagePlan"
  description = "Usage plan for Payment Service API"

  api_stages {
    api_id = aws_api_gateway_rest_api.payment_api_gateway.id
    stage  = aws_api_gateway_stage.dev_stage.stage_name
  }

  # Define throttling limits (e.g., 100 requests per second burst, 50 requests per second stable)
  throttle {
    burst_limit = 100
    rate_limit  = 50
  }

  # Define quota limits (e.g., 100000 requests per month)
  quota {
    limit  = 100000
    period = "MONTH"
    offset = 0
...
resource "aws_api_gateway_usage_plan_key" "payment_usage_plan_key" {
  ...
  key_type      = "API_KEY"
  usage_plan_id = aws_api_gateway_usage_plan.payment_usage_plan.id
}
...
output "api_key_value" {
  description = "The value of the API Key for Payment Service"
